...
#define Hardware_IIC_No hi2c1
#define IIC_MODE

void HALDreamCore::_ssd1306_transmit_cmd(unsigned char _cmd)
{ // NOLINT
#ifdef IIC_MODE
...
void HALDreamCore::_ssd1306_transmit_data(unsigned char _data, unsigned char _mode)
{ // NOLINT
  if (!_mode)
    _data = ~_data;
#ifdef IIC_MODE
...
void HALDreamCore::_ssd1306_reset(bool _state)
{ // NOLINT
  if (_state)
